...
COMMENT ON COLUMN public.concierge_trip_usage.used_count IS
  'Queries consumed for the (user_id, trip_id) pair. Canonical plan caps: free=3, explorer=25, frequent_chraveler=unlimited.';

COMMENT ON FUNCTION public.increment_concierge_trip_usage(TEXT, INTEGER) IS
  'Atomically increments per-trip concierge usage. p_limit should come from canonical plan map (free=3, explorer=25, frequent_chraveler=NULL).';
