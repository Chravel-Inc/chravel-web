...
export async function verifyConciergeTripAccess(
  ...
  const { data: membership, error: membershipError } = await supabase
    ...
    .eq('trip_id', tripId)
    .eq('user_id', userId)
    // Active membership only — a departed member (status = 'left') must not run
    // Concierge tools against a trip they left. Matches is_active_trip_member
    // (status IS NULL OR 'active'); keeps text + voice tool bridges aligned.
