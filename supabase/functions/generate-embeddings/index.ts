...
serve(async req => {
  ...
  try {
    ...
    console.log(
      `Processing embeddings for trip ${tripId}, sourceType: ${sourceType}, forceRefresh: ${forceRefresh}`,
    );
...
async function generateEmbeddings(sourceData: SourceData[]): Promise<any[]> {
  ...
  // Batch requests (max 100 per batch) routed through unified Gemini client
  for (let i = 0; i < sourceData.length; i += 100) {
    const batch = sourceData.slice(i, i + 100);

    console.log(
      `Processing batch ${Math.floor(i / 100) + 1} of ${Math.ceil(sourceData.length / 100)}`,
    );
    ...
    if (!Array.isArray(vectors) || vectors.length !== batch.length) {
      throw new Error(
        `Invalid embedding response shape: expected ${batch.length} vectors, received ${vectors?.length ?? 0}`,
      );
    }

    console.log(`Embedding provider=${embeddingResult.provider} model=${embeddingResult.model}`);
