...
serve(async req => {
  ...
  try {
    ...
    const { data: membership, error: membershipError } = await supabase
      ...
      .eq('trip_id', tripId)
      .eq('user_id', user.id)
      // Active membership only — a departed member (status = 'left') must not insert
      // receipts. Matches is_active_trip_member (status IS NULL OR 'active').
