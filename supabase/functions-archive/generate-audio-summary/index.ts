...
serve(async req => {
  ...
  try {
    ...
    // Check user quota
    const { data: quotaCheck, error: quotaError } = await supabase.rpc('check_audio_quota', {
      p_user_id: user_id,
    });
    ...
    if (quotaError || !quotaCheck) {
      return new Response(
        JSON.stringify({
          error: 'Audio generation quota exceeded. Upgrade your plan for more usage.',
        }),
    ...
    const { error: insertError } = await supabase.from('audio_summaries').insert({
      id: fileKey,
      user_id: user_id,
      trip_id: trip_id,
      source_url: url,
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert content summarizer. Create an engaging, conversational audio summary that sounds natural when spoken aloud. Focus on key insights, interesting details, and practical takeaways. Keep it concise but informative, suitable for a 2-3 minute audio overview.',
        },
        {
          role: 'user',
