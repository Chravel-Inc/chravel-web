...
// NOTE: despite the name, this function calls Google Cloud TTS (API-key based), not ElevenLabs.
...
const HARDCODED_PRIMARY_VOICE = 'en-US-Neural2-J';
const HARDCODED_FALLBACK_VOICE = 'en-US-Wavenet-D';
...
const toGoogleVoiceName = (rawVoice: string): string => {
  ...
  // Backward compatibility for legacy ElevenLabs IDs previously stored in app_settings.
  if (!trimmed.includes('-')) {
...
serve(async (req: Request) => {
  ...
  if (!GOOGLE_CLOUD_TTS_API_KEY) {
    console.error('[elevenlabs-tts] GOOGLE_CLOUD_TTS_API_KEY is not set');
  ...
  console.log(
    `[elevenlabs-tts] Request: voice=${resolvedVoiceId}, textLen=${text.length}, user=${user.id}`,
  );
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `[elevenlabs-tts] Primary voice ${resolvedVoiceId} returned ${ttsRes.status}: ${errBody}. Retrying with fallback voice ${fallbackVoice}`,
      );

      ttsRes = await callGoogleCloudTTS(text, fallbackVoice);
      usedFallback = true;
      console.log(`[elevenlabs-tts] Fallback voice result: status=${ttsRes.status}`);
    }
  } catch (fetchError) {
    console.error('[elevenlabs-tts] Fetch to Google Cloud TTS failed:', fetchError);
  ...
  if (!ttsRes.ok) {
    const errBody = await ttsRes.text().catch(() => '');
    console.error(`[elevenlabs-tts] Google Cloud TTS error ${ttsRes.status}: ${errBody}`);
  ...
  try {
    ...
    console.error('[elevenlabs-tts] Failed to parse Google TTS audio payload:', parseError);
