...
serve(async req => {
  ...
  try {
    ...
    const description = getMeta('og:description', 'twitter:description') || getMeta('description');

    let imageUrl = getMeta('og:image', 'twitter:image');

    const siteName = getMeta('og:site_name') || validUrl.hostname;
