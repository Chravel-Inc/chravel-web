...
function isJoinRequestApprovedNotification(notification: NotificationItem): boolean {
...
/**
 * In-app notifications list (join requests, chat, etc.).
 * Mounted from TripActionBar on lg+ viewports and from Index on mobile so the Alerts tab
 * always has a Dialog in the React tree (TripActionBar is hidden below lg).
 */
export const NotificationsDialog = ({ open, onOpenChange }: NotificationsDialogProps) => {
  ...
  const handleMarkAllAsRead = async () => {
    if (!isDemoMode && user) {
      await markAllAsRead(displayNotifications);
    }
  };

  const handleClearAll = async () => {
    if (!isDemoMode && user) {
      await clearAll(displayNotifications);
    }
  };
  ...
  const displayNotifications: NotificationItem[] = isDemoMode
