...
export class MockKnowledgeService {
  static async searchMockData(query: string, tripId: string): Promise<SearchResult[]> {
    ...
    // Mock poll data
    if (
      lowercaseQuery.includes('poll') ||
      lowercaseQuery.includes('restaurant') ||
      lowercaseQuery.includes('vote')
    ) {
      results.push({
        id: 'mock_poll_1',
        objectType: 'message',
    ...
    if (lowercaseQuery.includes('task') || lowercaseQuery.includes('list')) {
      results.push({
        ...
        tripName: 'Demo Trip',
        content:
          'Outstanding tasks: Pack snorkeling gear, Confirm dinner reservations, Buy sunscreen',
    ...
    if (
      ...
      lowercaseQuery.includes('itinerary')
    ) {
      results.push({
        id: 'mock_file_1',
        objectType: 'file',
  ...
  static async generateMockAnswer(query: string, tripId: string): Promise<string> {
    ...
    if (lowercaseQuery.includes('broadcast') || lowercaseQuery.includes('announcement')) {
      ...
      return (
        `📢 **Broadcast Messages** (${broadcasts.length} total)\n\n` +
        broadcasts
          .map(
            (b, i) =>
    ...
    if (
      ...
      (lowercaseQuery.includes('trip') ||
        lowercaseQuery.includes('coming') ||
        lowercaseQuery.includes('going'))
    ) {
      const memberList = mockMembers
        .map((m, i) => `- **${m.display_name}**${i === 0 ? ' (Organizer)' : ''}`)
        .join('\n');
    ...
    if (
      lowercaseQuery.includes('agenda') ||
    ...
    if (
      ...
      const paymentList = mockPayments
        .map(p => {
          ...
          const payer =
            mockMembers.find(m => m.user_id === p.created_by)?.display_name || 'Unknown';
    ...
    if (
      ...
      lowercaseQuery.includes('need to do')
    ) {
    ...
    if (
      ...
      lowercaseQuery.includes('landing') ||
      lowercaseQuery.includes('arrival')
    ) {
    ...
    if (
      ...
      lowercaseQuery.includes('directions')
    ) {
