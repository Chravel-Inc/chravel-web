...
| 2 | Scroll pricing | "Start free. Upgrade when you're ready for relief." `[OBSERVED — PricingSection.tsx]` |
...
| 7 | Join CTA copy | Always "Request to Join" + Member Approval notice by design (UI ignores invite flag) `[OBSERVED — JoinTrip.tsx:115-121,832; join-trip edge hardcodes approval — growth friction, not a copy bug]` |
| 8 | Smart Import | 1 free taste per trip, then paywall `[OBSERVED — MobileGroupCalendar.tsx:142-158]` |
| 9 | AI concierge | 3 AI queries/user/trip on free `[OBSERVED — PricingSection.tsx:76,241; billing entitlements ai_queries_basic]` |
...
| 6 | iOS subscribe | `APPLE_IAP_ENABLED: true` (native IAP path) `[OBSERVED — billing/config.ts:260; superseded June false claim — see REBASE-REFRESH]` |
...
5. **Guest read-only itinerary on invite preview** — approval-always is intentional; unlock schedule without full membership (`permissionMatrix` / `JoinTrip.tsx`).
...
| AI Concierge | 3 | [OBSERVED] | 3 free queries/trip — enough for a weekend if used carefully |
...
| Pricing tagline | "Start free. Upgrade when you're ready for relief." | [OBSERVED] |
...
| 6 | Concierge 3 AI queries/user/trip on free tier | `ground-truth §7` |
...
| 16 | `APPLE_IAP_ENABLED = true` (was false in June; refreshed) — iOS says subscribe on web | `ground-truth §7` |
