...
- **Budget sensitivity:** EXTREME. Will not spend $1 of his own money on software. The chapter
  treasury *might* approve a one-time expense if the treasurer signs off, but a personal $9.99/mo
...
`[OBSERVED — ground-truth §3]`. Friction: "group travel" framing; rush week is an *event*, and
...
`[OBSERVED — src/hooks/useInviteLink.ts:185-195]`. `JoinTrip` *can* render an `INVITE_MAX_USES`
...
invisible to everyone else. Verdict: PARTIAL — the edit is easy; the *delivery guarantee* isn't there.
...
Net: the chair can punish a spammer but cannot *prevent* chaos.
...
- **Day-7 retention: 3/10** — rush week *is* 7 days, and the notification architecture
...
1. **Replace the OS-mute death spiral with per-trip mute + an "announcement" priority class that
   bypasses it**, because Tucker failed at *delivering a venue change to 140 people on night two*
...
2. **Replace the invisible/unset `max_uses` with an exposed max-uses + one-tap revoke-and-reissue
   on the invite modal**, because Tucker failed at *containing a rush link reposted publicly*
...
3. **Replace consumer_member wildcard delete with a trip-level "restricted mode" (delete = author
   or admin)**, because Tucker failed at *keeping pranksters from nuking the schedule/Basecamp/polls*
...
4. **Replace one-by-one join approval with bulk approve + auto-approve-with-domain/code option, and
   ...
   *onboarding 140 people between events* (approval default ON, Event approvals chair-only
...
5. **Replace the `mailto:` Pro path and per-user Trip Passes with a self-serve, org-expensable
   ...
   Tucker failed at *converting at any cap he hit* — he won't email billing@chravelapp.com and won't
