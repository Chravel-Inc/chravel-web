...
test.describe('Trip RLS Policies', () => {
  ...
  test('RLS-TRIP-003: Creator can update trip', async ({
    ...
    const { error } = await client.from('trips').update({ name: 'Updated Name' }).eq('id', trip.id);

    // Should succeed
    expect(error).toBeNull();

    // Verify update
    const { data: updated } = await client.from('trips').select('name').eq('id', trip.id).single();
  ...
  test('RLS-TRIP-004: Non-admin member cannot update trip', async ({
    ...
    const { data } = await clientOwner.from('trips').select('name').eq('id', trip.id).single();
...
test.describe('Trip Members RLS Policies', () => {
  test('RLS-MEMBER-001: Non-member cannot see trip members', async ({
    ...
    const { data } = await clientOutsider.from('trip_members').select('*').eq('trip_id', trip.id);
  ...
  test('RLS-MEMBER-002: Member can see other members', async ({
    ...
    const { data } = await clientA.from('trip_members').select('user_id').eq('trip_id', trip.id);
...
test.describe('Chat Messages RLS Policies', () => {
  ...
  test('RLS-CHAT-004: Non-member cannot send messages', async ({
    ...
    const clientOutsider = await getClientAsUser(outsider);
    const { error } = await clientOutsider.from('trip_chat_messages').insert({
      trip_id: trip.id,
      user_id: outsider.id,
...
test.describe('Invite Links RLS Policies', () => {
  test('RLS-INVITE-001: Non-member cannot create invite', async ({
    ...
    const clientOutsider = await getClientAsUser(outsider);
    const { error } = await clientOutsider.from('invite_links').insert({
      trip_id: trip.id,
