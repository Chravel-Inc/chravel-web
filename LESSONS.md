...
When deleting one value from an enum-style `CHECK (col IN (...))` on a SHARED table (e.g. dropping `'sms'` from `notification_deliveries.channel`/`notification_logs.type`), leave the constraint permissive. Historical rows may still carry the value; tightening requires purging/migrating them and risks failing on prod data for zero gain. Stop *producing* the value at its source (the fan-out trigger) instead. *Evidence: SMS teardown left CHECKs widened; leftover queued sms rows were cancelled (prod `notification_delivery_status` has no `skipped`) via 20260730180203_remove_sms_twilio_leftovers.sql after 20260604120000 never landed on ChravelApp.*

### Repo migration ≠ applied to prod — verify schema_migrations before assuming teardown
Code can remove a channel while DB leftovers remain if an earlier migration never applied. Before claiming SMS/Twilio (or any infra) is gone, query `information_schema` + `supabase_migrations.schema_migrations` on the live project. Match MCP `apply_migration` timestamps to the repo filename so later `db push` does not double-apply under a different version. *Evidence: 20260604120000_remove_sms_notifications.sql sat unapplied while sms_* columns/RPCs remained; leftovers migration applied as version 20260730180203.*

### CREATE OR REPLACE of shared preference gates must copy the LATEST body
When dropping columns that a shared function reads, do not paste an older `CREATE OR REPLACE` from a previous teardown draft. Re-copy the current production body (or the newest migration that redefined the function) and only change the SMS-related arms. *Evidence: 20260730180203 first applied with DEFAULT 'push' and without mentions/in_app aliases, regressing the 20260708/09 preference contract until 20260730181722 restored it.*
...
### Verify reversible DDL migrations with a BEGIN/ROLLBACK dry-run via the Supabase MCP
Before committing a policy/grant/search_path migration, run the whole body wrapped in `BEGIN; ... ROLLBACK;` through `execute_sql`, then re-query state (`proconfig`, `has_*_privilege`, `pg_policies`) to confirm it both applied cleanly AND left prod untouched. This catches wrong function signatures, misspelled policy names, and missing schemas before CI — without mutating prod. Never run the DDL bare (it auto-commits). Evidence: 2026-07-21 advisor-hardening migration — all identifiers validated against live schema this way, prod state confirmed unchanged after.

### Triage "trusts p_user_id" SECURITY DEFINER RPCs by their EXECUTE grant, not just their body
When auditing SECURITY DEFINER functions that take a `p_user_id`/`target_user_id`, the fix depends on the ACL. Split them three ways: (a) "act as caller" RPCs (vote, toggle task, basecamp save) must `RAISE` on `auth.uid() IS NULL OR auth.uid() <> p_user_id`; (b) admin-action RPCs where the target is legitimately another user (promote/demote/assign_seat) must gate the ACTOR `auth.uid()` as admin — the target-as-param is intentional; (c) internal helpers granted to `anon`/`authenticated` but called only by service_role/other definers (sms counters, audit-log writers) should have the client grant REVOKED rather than a uid check bolted on (a service-role caller has `auth.uid() IS NULL`). Query `aclexplode(proacl)` to see which roles can actually reach each function — a body that trusts its param is only exploitable if `anon`/`authenticated` hold EXECUTE. **Critical gotcha: `REVOKE ... FROM anon, authenticated` is a NO-OP when the function also has a blanket PUBLIC grant** (`=X/...` as the first proacl entry — the Supabase default for RPCs). PUBLIC keeps granting EXECUTE to every role, so you must `REVOKE EXECUTE ... FROM PUBLIC, anon, authenticated` (the explicit `service_role` grant survives since it isn't PUBLIC). Always assert the result with `has_function_privilege('anon'/'authenticated'/'service_role', fn, 'EXECUTE')` in the BEGIN/ROLLBACK dry-run — don't assume the REVOKE worked. Same trap for `COALESCE(auth.uid(), param)` fallbacks: an unauthenticated PUBLIC caller has `auth.uid() IS NULL` and falls through to the spoofable param, so derive strictly from `auth.uid()` unless a trusted service_role path genuinely needs the fallback. *Evidence: 2026-07-23 actor-validation pass — `update_trip_basecamp_with_version` had zero auth check + PUBLIC EXECUTE (RLS bypass of the `trips` cover-edit policy); `get_visible_profile_fields(victim,victim)` unmasked PII via trusted `requesting_user_id`; the first-pass `REVOKE FROM anon, authenticated` on the SMS/audit helpers did nothing until PUBLIC was added.*
