...
### Remotion in the remote container: chrome-for-testing mode, and never delayRender fonts at module scope
The pre-installed Chromium (`/opt/pw-browsers/chromium`) rejects Remotion's default old-headless launch ("Old Headless mode has been removed") — pass `--chrome-mode=chrome-for-testing` alongside `--browser-executable`. Google Fonts fails inside the render browser with `ERR_CERT_AUTHORITY_INVALID` (agent-proxy MITM cert), so fonts must be network-free. The trap: the bundle is ALSO evaluated on Remotion's composition-selection page, where font promises never resolve and timers never fire — any module-scope `delayRender` (including `@remotion/fonts loadFont()`) is never cleared there and detonates after its timeout, killing the render at exactly render-speed × timeout frames (four failed runs died at frames 327/1206/1122/311 under 28s/28s/108s/28s fuses; `remotion still` masks the bug because it exits before the fuse). Working recipe: curl the woff2 once, embed it as a base64 data-URI module, register it with a synchronous `@font-face` style tag, and use no delayRender at all — data-URI fonts decode during style resolution. *Evidence: July 2026 comparison-video renders; clean 1500-frame render only after removing every font delayRender.*

### Verify Remotion choreography with stills at hand-computed frames, including just after Sequence cuts
`npx remotion still --frame=N` at mid-swipe, mid-transition, chip-reveal, and scene-boundary frames caught chip-row misalignment (hidden flex children reserving layout space), duplicate home-screen icons across pages, and a scene-cut "rewind" (re-driving a component with a frame offset restarts its springs — prefer an explicit terminal-state prop like `overviewOnly` over replaying with `frame + OFFSET`). Compute the interesting frame numbers from the timing constants rather than sampling evenly. *Evidence: July 2026 comparison video — stills at f130/f200/f215/f250/f560/f830/f1130 drove all visual fixes before the first full render.*
