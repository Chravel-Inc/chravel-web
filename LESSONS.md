...
### Filtering notifications by category: trip-acceptance hides under a generic type
"You were accepted on a trip you requested" is written by the `approve_join_request` RPC with the generic `type='success'` plus `metadata.action='join_approved'` (the alternate `approve-join-request` edge writer uses `type='join_approved'`). Any category filter that matches on `type` alone silently misses 100% of acceptances. Match on `metadata->>'action'='join_approved'` OR `type='join_approved'`. Verified against the live DB during the app-icon badge work: the metadata clause returned 14 unread acceptances where a type-only filter returned 0. *Evidence: PR #674 badge categories — `src/lib/notifications/badgeCategories.ts` + `supabase/functions/_shared/badgeCategories.ts`.*

### App-icon badge needs a recall path independent of push delivery
A category badge that only reflects pushed notifications under-counts whenever a category is push-suppressed (chat is a `SUPPRESSED_CATEGORY`) or a push fails. Drive the badge from a count query over the `notifications` table (PWA `navigator.setAppBadge` in `useAppBadge`, recomputed on realtime read/insert/clear + app foreground), and have the server inject the same count into push payloads (`apns.payload.aps.badge`, web `data.badgeCount`) as a fast-path. On a count-query error return `null` and OMIT the badge field — never send `0`, which clears a still-valid icon. Mount the badge hook at the app shell (`AppInitializer`), not a single route, or it stops reconciling elsewhere. *Evidence: PR #674.*
