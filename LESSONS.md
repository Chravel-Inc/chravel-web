...
## 2026-06-11 — Synthetic research evidence boundaries

When generating synthetic persona research, keep a hard boundary between code-grounded observations and simulated customer reactions. Use `[OBSERVED]` only for source files, existing docs, local app behavior, browser/console/network evidence, or other directly inspected artifacts; downgrade persona reactions and pricing intent to `[SIMULATED RISK]` or `[HYPOTHESIS]`. For investor-facing research packages, include parseable CSV/JSON outputs and validation scripts/counts so the artifact can drive implementation tickets without being mistaken for real customer proof.
