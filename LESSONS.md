...
When auditing SECURITY DEFINER functions that take a `p_user_id`/`target_user_id`, the fix depends on the ACL. Split them three ways: (a) "act as caller" RPCs (vote, toggle task, basecamp save) must `RAISE` on `auth.uid() IS NULL OR auth.uid() <> p_user_id`; (b) admin-action RPCs where the target is legitimately another user (promote/demote/assign_seat) must gate the ACTOR `auth.uid()` as admin — the target-as-param is intentional; (c) internal helpers granted to `anon`/`authenticated` but called only by service_role/other definers (sms counters, audit-log writers) should have the client grant REVOKED rather than a uid check bolted on (a service-role caller has `auth.uid() IS NULL`). Query `aclexplode(proacl)` to see which roles can actually reach each function — a body that trusts its param is only exploitable if `anon`/`authenticated` hold EXECUTE. *Evidence: 2026-07-23 actor-validation pass — `update_trip_basecamp_with_version` had zero auth check + anon EXECUTE (RLS bypass of the `trips` cover-edit policy); `get_visible_profile_fields(victim,victim)` unmasked PII via a trusted `requesting_user_id`; SMS/audit helpers were grant-revoked, not uid-checked.*
